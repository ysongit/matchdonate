...
contract GiftBox {
    ...
    event GiftCancelled(uint256 indexed giftId, address indexed sender, uint256 amount, uint256 timestamp);
    ...
    function claimGift(string memory redeemCode) external whenNotPaused returns (uint256) {
        ...
        require(IERC20(gift.tokenAddress).transfer(msg.sender, gift.amount), "Token transfer failed");
